...
class ResultClass {
	...
	public static List<String> fetchItemsToDisplay(List<List<String>> items, int sortParameter, int sortOrder,
		...
		for (int i = itemPerPage * pageNumber; i < Math.min((itemPerPage * pageNumber) + itemPerPage,
			...
			retList.add(itemList.get(i).getVal());
		}
...
public class PaginationJava8PartialExecution {
	public static void main(String[] args) throws IOException {
		...
		List<String> result = ResultClass.fetchItemsToDisplay(items, sortParameter, sortOrder, itemPerPage, pageNumber);
